...
function normalizeAddr(addr) {
  ...
  if(addr && addr.indexOf('::ffff:') === 0) return addr.substring(7);
  return addr;
}
...
function makeWsTransport(options, callback) {
  ...
  function init(ws) {
    var remote = {address: normalizeAddr(ws._socket.remoteAddress), port: ws._socket.remotePort},
        local = {address: normalizeAddr(ws._socket.address().address), port: ws._socket.address().port},
        flowid = [remote.address, remote.port, local.address, local.port].join();

    console.log('[WS] Connection init - flowid:', flowid, 'remote:', remote.address + ':' + remote.port, 'local:', local.address + ':' + local.port);
    flows[flowid] = ws;
    ...
    ws.on('message', function(data, isBinary) {
      ...
      // WebSocket is message-framed, so some clients don't send the
      // trailing \r\n\r\n that the SIP parser requires. Append it if missing.
      if(str.indexOf('\r\n\r\n') === -1) {
        if(str.endsWith('\r\n')) {
          str += '\r\n';
      ...
      if(msg) {
        ...
        console.log('[WS] Failed to parse SIP message - start:', JSON.stringify(str.substring(0, 100)));
      }
    });
  }
